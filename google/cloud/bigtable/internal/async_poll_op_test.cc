...
class NoexTableAsyncPollOpTest : public bigtable::testing::TableTestFixture {};
...
class DummyOperation {
 public:
  ...
  explicit DummyOperation(std::shared_ptr<DummyOperationImpl> impl)
      : impl_(std::move(impl)) {}
...
TEST_P(NoexTableAsyncPollOpImmediateFinishTest, ImmediateFinish) {
  ...
  auto polling_policy = bigtable::DefaultPollingPolicy(
      config.poll_exhausted ? no_retries : internal::kBigtableLimits);
  ...
  auto cq_impl = std::make_shared<testing_util::FakeCompletionQueueImpl>();
  ...
  /*[&user_op_completed, config](CompletionQueue&, int& response,
                               grpc::Status& status) {
...
*/
  EXPECT_CALL(*dummy_op_mock, Start(_, _, _))
      .WillOnce([&on_dummy_op1_finished](CompletionQueue&,
                                         std::unique_ptr<grpc::ClientContext>&,
                                         Functor const& callback) {
        on_dummy_op1_finished = callback;
        return std::shared_ptr<AsyncOperation>(new AsyncOperationMock);
      })
      .WillOnce([&on_dummy_op2_finished](CompletionQueue&,
                                         std::unique_ptr<grpc::ClientContext>&,
                                         Functor const& callback) {
...
TEST_F(NoexTableAsyncPollOpTest, CancelBeforeStart) {
  ...
  /*[&user_op_completed](CompletionQueue&, int& response, grpc::Status& status)
  { EXPECT_FALSE(status.ok()); EXPECT_EQ(grpc::StatusCode::CANCELLED,
  status.error_code()); EXPECT_EQ(27, response); user_op_completed.set_value();
  };*/
...
TEST_P(NoexTableAsyncPollOpCancelInOpTest, CancelInOperation) {
  ...
  // doesn't allow any retries (as specified by `no_retries`). We use it when
...
*/
  EXPECT_CALL(*dummy_op_mock, Start(_, _, _))
      .WillOnce([&on_dummy_op_finished, &dummy_op_handle_mock](
                    CompletionQueue&, std::unique_ptr<grpc::ClientContext>&,
                    Functor const& callback) {
        on_dummy_op_finished = callback;
  ...
  EXPECT_CALL(static_cast<AsyncOperationMock&>(*dummy_op_handle_mock), Cancel())
      .WillOnce([&cancel_called]() { cancel_called = true; });
...
TEST_F(NoexTableAsyncPollOpTest, TestRealTimerCancellation) {
  ...
  internal::RPCPolicyParameters const inifinite_poll = {
      std::chrono::hours(100),
      std::chrono::hours(1000),
      std::chrono::hours(10000),
  };
  auto polling_policy = bigtable::DefaultPollingPolicy(inifinite_poll);
  MetadataUpdatePolicy metadata_update_policy(kTableId,
                                              MetadataParamTypes::TABLE_NAME);
  ...
  /*[&user_op_completed_promise](CompletionQueue&, int& response,
                               grpc::Status& status) {
...
TEST_P(NoexTableAsyncPollOpCancelInTimerTest, TestCancelInTimer) {
  ...
  /*[&user_op_completed](CompletionQueue&, int& response, grpc::Status& status)
  { EXPECT_FALSE(status.ok()); EXPECT_EQ(grpc::StatusCode::CANCELLED,
  status.error_code()); EXPECT_EQ(27, response); user_op_completed = true;
  };*/
 ...
 */
  ...
  cq_impl->SimulateCompletion(!notice_cancel);
...
class NoexTableAsyncPollOpImmediateFinishFutureTest
 ...
 public:
  NoexTableAsyncPollOpImmediateFinishFutureTest()
      ...
      : cq_impl_(std::make_shared<
                 bigtable::testing_util::FakeCompletionQueueImpl>()),
        cq_(cq_impl_),
  ...
  std::shared_ptr<testing_util::FakeCompletionQueueImpl> cq_impl_;
  CompletionQueue cq_;
